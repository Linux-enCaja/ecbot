...
a funcin mmap intenta ubicar length bytes comenzando en  el  desplaza-
       miento  offset  desde  el  fichero  (u otro objeto) especificado por el
       descriptor de fichero fd en memoria,  preferiblemente  en  la  direccin
       start.   Esta ltima direccin es una sugerencia y normalmente se especi-
       fica como 0.  El lugar donde es ubicado el objeto es devuelto por mmap,
       y  nunca  vale  0.   El argumento prot describe la proteccin de memoria
       deseada. (y no debe entrar en conflicto con el  modo  de  apertura  del
       fichero).  Puede  valer  PROT_NONE  o  ser  la  combinacin  mediante la
       operacin OR de una o ms de las otras banderas PROT_*.
...
{	
	...
	for (i = 0 ; i < 0xfff; i++)
	{
          virt_addr[i<<10] = i*4;       // ECBOT use from A11 to A25
	}
        #endif
