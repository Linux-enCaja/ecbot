...
void I2cSetSlaveAddress( int i2cDev, I2C_Addr_t i2cAddr, int useCrc )
{
    ...
    {
#ifdef DEBUG
        printf( "I2cSetSlaveAddress: Error trying to set slave address to 0x%02x (%d %s)\n", 
    ...
    {
        printf( "I2cSetSlaveAddress: Error trying to set PEC mode\n" );
    }
#endif
...
)
{
    ...
    if ( wrData != NULL )
    {
        //printf( "----- wrData:0x%08x *wrData:0x%02x -----\n", wrData, *(const uint8_t *)wrData ); 
    }
    ...
    {
#ifdef DEBUG
        printf( "I2cTransfer: wrLen too big: %d, max is %d\n",
    ...
    {
#ifdef DEBUG
        printf( "I2cTransfer: rdLen too big: %d, max is %d\n",
...
#ifdef DEBUG
        printf( "msg[ 0 ].addr  = 0x%02x\n", msg[ 0 ].addr );
        printf( "msg[ 0 ].flags = 0x%04x\n", msg[ 0 ].flags );
        printf( "msg[ 0 ].len   = %d\n",     msg[ 0 ].len );
    ...
    {
#ifdef DEBUG
        printf( "I2cTransfer: ioctl failed: %s (%d)\n", strerror( errno ), errno );
#endif
        return -1;
    }
    ...
    if ( rdLen > 0 )
    {
        if ( rdBlock )
        {
            ...
            {
#ifdef DEBUG
                printf( "I2cTransfer: length is too big: %d max: %d\n", rdBuf[ 0 ], rdLen );
#endif
                rc = EMSGSIZE;
            }
            else
        ...
        if ( gUseCrc )
        {
            ...
            {
#ifdef DEBUG
                printf( "I2cTransfer: CRC failed: Rcvd: 0x%02x, expecting: 0x%02x\n",
